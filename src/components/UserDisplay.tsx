...
import { UserContext } from "../../../simpleapi-ts/src/App"; //userdata objecct from the api has been sent via createContext/useContext
...
React.useEffect(()=> {
  ...
  if(gender!=='all'&&location==='all'){
    Data=usersData.filter((e:any)=>e.gender===gender);
    setDisplayList(Data);
  }
  else if(gender!=='all'&&location!=='all'){
    Data=usersData.filter((e:any)=>e.gender===gender&&e.nat===location);
    setDisplayList(Data);
  }
  else if(gender==='all'&&location!=='all'){
    Data=usersData.filter((e:any)=>e.nat===location);
    setDisplayList(Data);
  }
  else{
